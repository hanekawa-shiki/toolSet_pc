...
async function handleFiles() {
  ...
  for (const file of selectedFiles.value) {
    try {
      ...
      setMessage(message, { type: 'error', msg: error.message })
...
async function handleCopy(link: string) {
  try {
    ...
    setMessage(message, { type: 'success', msg: '磁力链接已复制' })
  } catch {
    setMessage(message, { type: 'warning', msg: '复制失败，请手动复制' })
  }
}
...
async function copyAll() {
  ...
  try {
    ...
    setMessage(message, { type: 'success', msg: '已复制全部磁力链接' })
  } catch {
    setMessage(message, { type: 'warning', msg: '复制失败，请手动复制' })
  }
}
...
async function export2file() {
  if (fileDetails.value.length === 0) {
    setMessage(message, { type: 'warning', msg: '没有可以导出的磁力链接' })
    return
  }
  ...
  setMessage(message, { type: 'success', msg: '磁力链接已导出' })
}
