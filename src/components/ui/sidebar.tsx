...
function SidebarProvider({
  ...
  const setOpen: (value: boolean | ((value: boolean) => boolean)) => void = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // 将侧边栏状态保存到 localStorage 中。
      localStorage.setItem(SIDEBAR_LOCAL_STORAGE_KEY, String(newOpen));
    },
    [setOpenProp, setOpenRaw, openState],
  );

  // 切换侧边栏的辅助方法。
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile(o => !o) : setOpenRaw(o => !o);
  }, [isMobile, setOpenRaw, setOpenMobile]);
  ...
  // 添加一个状态，以便可以使用 data-state="expanded" 或 "collapsed"。
  // 这使得使用 Tailwind 类来设置侧边栏样式更加方便。
